...
class MessageListManagerTest {
    ...
    fun `addRemoveReaction() should add new reaction if not already added`() = testDispatcher.runBlockingTest {
        ...
        val expectedReactions = Reaction.values().toList().getExpectedReactions(listOf(participantIdentity))
        var actualReactions = ReactionAttributes()

        coEvery { message.participantSid } returns participantIdentity
        coEvery { participant.sid } returns message.participantSid
        coEvery { conversation.getMessageByIndex(any()) } returns message
        ...
        Reaction.values().forEach { reaction ->
            messageListManager.addRemoveReaction(messageIndex, reaction)
        }
    ...
    fun `addRemoveReaction() should update reaction counter if already added but not by self user`() = runBlockingTest {
        val existingParticipant = "existing_participant"
        val messageIndex = 1L
        val message = mockk<Message>()
        val reactions: MutableMap<String, Set<String>> = mutableMapOf()
        val initialReactions = listOf(Reaction.HEART).getExpectedReactions(listOf(existingParticipant))
        val expectedReactions = listOf(Reaction.HEART).getExpectedReactions(listOf(existingParticipant, participantIdentity))
        ...
        messageListManager.addRemoveReaction(messageIndex, Reaction.HEART)
        assertEquals(expectedReactions, actualReactions)
    }
    ...
    fun `addRemoveReaction() should remove reaction if added by self user`() = runBlockingTest {
        ...
        val message = mockk<Message>()
        val reactions: MutableMap<String, Set<String>> = mutableMapOf()
        val initialReactions = listOf(Reaction.HEART).getExpectedReactions(listOf(existingParticipant, participantIdentity))
        val expectedReactions = listOf(Reaction.HEART).getExpectedReactions(listOf(existingParticipant))
        var mockAttributes = Attributes(JSONObject(Gson().toJson(initialReactions)))
        var actualReactions = ReactionAttributes()
    ...
    fun `addRemoveReaction() should not be updated on error`() = runBlockingTest {
        ...
        coEvery { participant.sid } returns message.participantSid
        coEvery { conversation.getMessageByIndex(any()) } returns message
        coEvery { conversationsClient.getConversation(any()) } throws ConversationsException(ConversationsError.REACTION_UPDATE_FAILED)
        try {
            messageListManager.addRemoveReaction(messageIndex, Reaction.HEART)
        } catch (e: ConversationsException) {
            assert(ConversationsError.REACTION_UPDATE_FAILED == e.error)
        }
    }
